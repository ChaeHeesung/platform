...
<body id="main">
	...
	<table table id="rec" class="table table-hover table-striped table-bordered" style="width: 100%;">
		<tr>
			<th class="border-top border-bottom border-left border-right">자치구명</th>
			<th class="border-top border-bottom border-right">도서관명</th>
			...
			<th class="border-top border-bottom border-right">추천년도</th>
			<th class="border-top border-bottom border-right">추천월</th>
		</tr>
		<c:forEach items="${list}" var="l">
			<tr>
				<td class="border-bottom border-left border-right">${l.gu}</td>
				<td class="border-bottom border-right">${l.rlibrary}</td>
				<td class="border-bottom border-right">${l.rbook}</td>
				<td class="border-bottom border-right">${l.rwriter}</td>
				<td class="border-bottom border-right">${l.rpublisher}</td>
				<td class="border-bottom border-right">${l.ryear}</td>
				<td class="border-bottom border-right">${l.rmon}</td>
			</tr>
		</c:forEach>
	</table>
	...
	<ul class="pagination">
		<c:if test="${PagingUtil.prev}">
			<li><a href="recommend${PagingUtil.makeQuery(PagingUtil.startPage-1)}">&laquo;</a></li>
		</c:if>
		
		<c:forEach begin="${PagingUtil.startPage}" end="${PagingUtil.endPage}" var = "idx">
			<li>
				<a href="recommend${PagingUtil.makeQuery(idx)}">${idx}</a>
			</li>
		</c:forEach>
		
		<c:if test="${PagingUtil.next && PagingUtil.endPage > 0 }">
			<li><a href="recommend${PagingUtil.makeQuery(PagingUtil.endPage +1)}">&raquo;</a></li>
		</c:if>
	</ul>
</div>
</body>
